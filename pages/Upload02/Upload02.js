      ...
      that.getBaiduToken().then(res => {
        ...
        wx.request({
          ...
          success: function (res, resolve) {
            ...
            // that.setData({
            //   ConTent01:res.data.words_result
            // })
            console.log(that.data.ConTent01)
            for (let index = 0; index < res.data.words_result.length; index++) {
              that.data.ConTent01+=res.data.words_result[index].words
            ...
            wx.request({
              ...
              data: {
                'Content-Type':'application/x-www-form-urlencoded; charset=utf-8',
                text: that.data.ConTent01,//这里是需要进行关键词搜索的文档，你可以用选择文件的方法，把txt的文字提取来之后，再通过传参传到这里
              },
              success: function(res) {
        ...
        for (let index = 0; index < res.data.words_result.length; index++) {
          that.data.ConTent+=res.data.words_result[index].words
          console.log(res.data.words_result[index].words)
        }
